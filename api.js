/**
 * Person Tracker API Worker
 * Provides REST API for D1 database
 */

const DB_NAME = 'person-tracker-db';
  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (path === '/goals' && method === 'GET') {
        response = await env.DB.prepare('SELECT * FROM goals ORDER BY createdAt DESC').all();
      ...
      if (path === '/goals' && method === 'POST') {
        ...
        `).bind(title, description, goalType, status || 'in_progress', progress || 0, startDate, endDate, readingTime || 0, totalReadingTime || null).run();
      ...
      if (path.match(/^\/goals\/(\d+)\/milestones$/) && method === 'GET') {
        ...
        const milestones = await env.DB.prepare('SELECT * FROM milestones WHERE goalId = ? ORDER BY createdAt ASC').bind(goalId).all();
      ...
      if (path.match(/^\/milestones\/(\d+)$/) && method === 'PUT') {
        ...
        await env.DB.prepare(`
          UPDATE milestones SET title = ?, completed = ?, dueDate = ? WHERE id = ?
        `).bind(title, completed ? 1 : 0, dueDate || null, id).run();
      ...
      if (path.match(/^\/goals\/(\d+)\/logs$/) && method === 'GET') {
        ...
        const logs = await env.DB.prepare('SELECT * FROM dailyLogs WHERE goalId = ? ORDER BY date DESC').bind(goalId).all();
      ...
      if (path.match(/^\/goals\/(\d+)\/quotes$/) && method === 'GET') {
        ...
        const quotes = await env.DB.prepare('SELECT * FROM quotes WHERE goalId = ? ORDER BY createdAt DESC').bind(goalId).all();
      ...
      if (path === '/all-logs' && method === 'GET') {
        const logs = await env.DB.prepare('SELECT * FROM dailyLogs ORDER BY date DESC').all();
        return new Response(JSON.stringify(logs.results), {
          headers: { 'Content-Type': 'application/json', ...corsHeaders },
        });
      }
