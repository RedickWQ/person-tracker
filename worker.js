  ...
  async fetch(request, env) {
    ...
    if (path.startsWith('/api')) {
      ...
      try {
        ...
        if (apiPath.match(/^\/goals\/(\d+)$/) && method === 'GET') {
          const id = apiPath.match(/^\/goals\/(\d+)$/)[1];
        ...
        if (apiPath.match(/^\/goals\/(\d+)$/) && method === 'PUT') {
          const id = apiPath.match(/^\/goals\/(\d+)$/)[1];
          const body = await request.json();
          const updates = [];
          const values = [];
        ...
        if (apiPath.match(/^\/goals\/(\d+)$/) && method === 'DELETE') {
          const id = apiPath.match(/^\/goals\/(\d+)$/)[1];
        ...
        // Milestones
        if (apiPath.match(/^\/goals\/(\d+)\/milestones$/) && method === 'GET') {
          const goalId = apiPath.match(/^\/goals\/(\d+)\/milestones$/)[1];
        ...
        if (apiPath.match(/^\/goals\/(\d+)\/milestones$/) && method === 'POST') {
          const goalId = apiPath.match(/^\/goals\/(\d+)\/milestones$/)[1];
          const body = await request.json();
        ...
        if (apiPath.match(/^\/milestones\/(\d+)$/) && method === 'PUT') {
          const id = apiPath.match(/^\/milestones\/(\d+)$/)[1];
          const body = await request.json();
        ...
        if (apiPath.match(/^\/milestones\/(\d+)$/) && method === 'DELETE') {
          const id = apiPath.match(/^\/milestones\/(\d+)$/)[1];
        ...
        // Daily Logs
        if (apiPath.match(/^\/goals\/(\d+)\/logs$/) && method === 'GET') {
          const goalId = apiPath.match(/^\/goals\/(\d+)\/logs$/)[1];
        ...
        if (apiPath.match(/^\/goals\/(\d+)\/logs$/) && method === 'POST') {
          const goalId = apiPath.match(/^\/goals\/(\d+)\/logs$/)[1];
          const body = await request.json();
          const result = await env.DB.prepare(`
        ...
        if (apiPath.match(/^\/logs\/(\d+)$/) && method === 'DELETE') {
          const id = apiPath.match(/^\/logs\/(\d+)$/)[1];
        ...
        // Quotes
        if (apiPath.match(/^\/goals\/(\d+)\/quotes$/) && method === 'GET') {
          const goalId = apiPath.match(/^\/goals\/(\d+)\/quotes$/)[1];
        ...
        if (apiPath.match(/^\/goals\/(\d+)\/quotes$/) && method === 'POST') {
          const goalId = apiPath.match(/^\/goals\/(\d+)\/quotes$/)[1];
          const body = await request.json();
          const result = await env.DB.prepare(`
        ...
        if (apiPath.match(/^\/quotes\/(\d+)$/) && method === 'DELETE') {
          const id = apiPath.match(/^\/quotes\/(\d+)$/)[1];
        ...
        // All logs
        if (apiPath === '/all-logs' && method === 'GET') {
